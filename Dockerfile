...
COPY ./SMARTS/requirements.txt /tmp/requirements.txt
...
RUN pip install --no-cache-dir -e .[train,test,dev,camera-obs] \
    && cp -r ./smarts.egg-info /media/smarts.egg-info
...
RUN echo "mkdir -p /dev/input\n" \
         "if [[ ! -d /src/SMARTS/smarts.egg-info ]]; then" \
         "   cp -r /media/smarts.egg-info /src/SMARTS/smarts.egg-info;" \
         "   chmod -R 777 /src/SMARTS/smarts.egg-info;" \
         "fi" >> ~/.bashrc
